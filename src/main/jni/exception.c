...
//nullchecks just to prevent segfaults, LinkageError will be thrown if GifIOException cannot be instantiated
	...
	char errnoMessage[NL_TEXTMAX]="";
	strerror_r(errno, errnoMessage, NL_TEXTMAX);
	jstring message = (*env)->NewStringUTF(env,errnoMessage);
	jobject exception = (*env)->NewObject(env, exClass, mid, errorCode, message);
	if (exception != NULL) {
